...
namespace EndlessChase.Traffic
{
    ...
    public sealed class TrafficSpawner : MonoBehaviour
    {
        ...
        [SerializeField] float _policeChance = 0.12f;
        ...
        void SpawnForwardTraffic()
        {
            ...
            bool police = Random.value < _policeChance;
            string id = police
                ? _policePoolId
                : _civilianPoolIds[Random.Range(0, _civilianPoolIds.Length)];
            ...
            float speed = police ? _player.ForwardSpeed * 0.92f : Random.Range(6f, 12f);
            vehicle.Activate(police ? TrafficKind.Police : TrafficKind.Civilian, lane, speed, false, new Vector3(x, 0.4f, z));
        }
